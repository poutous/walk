...
var _1b3=_4d(this,opts.rownumbers,opts);
if(!opts.columns){
...
bindExport:function(datagrid, opts){//绑定导出
	...
	if (exportId) {
		//绑定目标导出对象
		$("#" + exportId).unbind('click'); //先解再绑 防止重复绑定
		$("#" + exportId).bind('click',function() {
			if($(this).attr("isAsyn")){
				opts.isAsyn = $(this).attr("isAsyn");
			}
			if($(this).attr("exportName")){
				opts.exportName = $(this).attr("exportName");
			}
	...
	if (exportCurrentId) {
		//绑定目标导出对象
		$("#" + exportCurrentId).unbind('click'); //先解再绑 防止重复绑定
		$("#" + exportCurrentId).bind('click',function() {
			if($(this).attr("exportName")){
				opts.exportName = $(this).attr("exportName");
			}
...
exportData:function(jq, opts){
	return jq.each(function(){
		...
		if(isAsyn == "true"){
			...
			$.messager.prompt("对话框", promptTitle, function(val){
				val = $.trim(val);
				if(!val){
					$.messager.alert("错误", promptTitle, "error");
					return false;
				}
				...
				if(queryParams) {
					...
					if (typeof(queryParams) == "object" && Object.prototype.toString.call(queryParams).toLowerCase() == "[object object]" && !queryParams.length) {
						...
						queryParams.headerJSON = headerJSON;
						queryParams.__actionType = 'asynExport';
					}
				} else {
					queryParams = {asynExportName : val, headerJSON : headerJSON, __actionType : 'asynExport'}
				}
		...
		//同步方式导出
			...
			$.messager.confirm("确认", "确定导出吗？", function(ok){
				if(ok){
					...
					//提交
					$('#tempDiv').remove();
					$("<div id='tempDiv'><iframe name='IF_4down'></iframe></div>").appendTo($(document.body)).css('display', 'none').append('<form id="tempForm" action="' + url + '" target="IF_4down" method="post" accept-charset="UTF-8">' + hiddenElements + '<input type="hidden" name="__actionType" value="export"/>' + '<input type="hidden" name="headerJSON" value=\'' + headerJSON + '\'/>' + '<input type="hidden" id="exportName" name="exportName"/></form>');
...
var _title='';
if(col.tip==true && _223){
	try{
		_title = _223.replace(/"/g, '&quot;').replace(/'/g, "&apos;");
	}catch(e){}
}
