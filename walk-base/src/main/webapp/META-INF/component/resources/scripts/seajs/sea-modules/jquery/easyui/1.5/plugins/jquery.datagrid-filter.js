(function($){
	...
	function resizeFilter(target, field, width){
		...
		ff.each(function(){
			...
			if (width != undefined){
				this.filter.resize(this, width-5);
			} else {
				this.filter.resize(this, 10);
				this.filter.resize(this, cc.width() - btn._outerWidth() - 5);
			}
		});
	}
	...
	function init(target, filters){
		...
		function initCss(){
			if (!$('#datagrid-filter-style').length){
				$('head').append(
					...
					'.datagrid-filter-row input{margin:0;padding: 5px 3px 5px 0px;-moz-border-radius:2px;-webkit-border-radius:2px;border-radius:2px;border:solid 1px #ffffff;height: 30px;}' +
					'</style>'
				);
			}
		}
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (field == '_'){continue;}
				var div = $('<div class="datagrid-filter-c" style="text-align:center"></div>').appendTo(td);
